...
use Police::Dispatch::Gui::MainFrame;
use Police::Dispatch::Gui::UnitPanel;
use Police::Dispatch::Gui::CallPanel;
...
sub OnInit {
    ...
    $self->{'frame'} = Police::Dispatch::Gui::MainFrame->new( 'Police Dispatch',
                                                              [ 200, 600 ] );
    $self->{'frame'}->Show;

    # $unitpanel = Police::Dispatch::Gui::UnitPanel->new( undef, -1, 'Police Dispatch' );
    # $self->{'frame'}->Show;
    # $self->add_menu_bar();

    # $self->{'frame'} = Police::Dispatch::Gui::CallFrame->new( undef, -1, 'Calls' );
    # $self->{'frame'}->Show;
...
sub run {
  ...
  my $app = Police::Dispatch::Gui->new();  
  $app->MainLoop;
